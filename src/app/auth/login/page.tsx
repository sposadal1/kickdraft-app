  ...
  async function handleRecuperar(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) {
        ...
        setMensajeExito('Te enviamos un enlace de recuperación. Revisa tu bandeja de entrada (y la carpeta de spam).');
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-verde-900/40 rounded-2xl mb-4">
            {modoRecuperar ? <Mail className="w-8 h-8 text-verde-400" /> : <Trophy className="w-8 h-8 text-verde-400" />}
          </div>
          <h1 className="text-2xl font-bold text-white">
            {modoRecuperar ? 'Recuperar contraseña' : 'Iniciar sesión'}
          </h1>
          <p className="text-gray-400 mt-1">
            {modoRecuperar ? 'Te enviaremos un enlace para restablecer tu contraseña' : 'Bienvenido de vuelta a Kickdraft'}
          </p>
        </div>

        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8">
          {error && <p className="text-red-400 text-sm text-center mb-4">{error}</p>}
          {mensajeExito && <p className="text-verde-400 text-sm text-center mb-4">{mensajeExito}</p>}
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <div className="text-right">
                  <button
                    ...
                    className="text-sm text-verde-400 hover:text-verde-300 transition-colors"
                  >
