...
export default function Navbar() {
  ...
  async function fetchNombre(userId: string) {
    const { data, error } = await supabase
      .from('perfiles')
      .select('nombre, apellido, nombre_visible, avatar_url')
      .eq('id', userId)
      .single();

    if (error) {
      console.error(
        'Navbar: error al cargar perfil:',
        error.message
      );
      return;
    }
      ...
      <nav className="bg-black text-white shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center h-16">
            ...
            <Link
              href="/"
              className="flex items-center gap-2 font-bold text-xl text-verde-500 flex-none"
            >
            ...
            <div className="hidden md:flex items-center">
              {usuario ? (
                ...
                >
                  ...
                  >
                    <div className="w-8 h-8 rounded-full bg-verde-600 flex items-center justify-center text-sm font-bold text-white">
                      {(nombreUsuario ||
                        usuario.email ||
                        'U')
                        .substring(0, 2)
                  ...
                  {perfilAbierto && (
                    <div className="absolute right-0 top-full mt-2 w-64 bg-gray-900 border border-gray-700 rounded-xl shadow-xl z-50 p-2">
                      ...
                      <Link
                        ...
                        className="block px-3 py-2 rounded-lg hover:bg-gray-800 text-sm text-white"
                      >
                        Mi perfil
                      </Link>
                      ...
                      <button
                        ...
                        className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-800 text-red-400 text-sm"
                      >
                        Cerrar sesión
                      </button>
                    </div>
            ...
            <div className="flex md:hidden items-center gap-2">
              {nombreUsuario && (
                <span className="text-gray-300 text-xs truncate max-w-[110px]">
                  {nombreUsuario}
                </span>
              )}
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-gray-900 border border-gray-700 rounded-xl shadow-xl z-50 p-3 space-y-2">
                    ...
                    {usuario ? (
                      <>
                        ...
                        <Link
                          href="/perfil"
                          className="block text-white text-sm py-2 hover:text-verde-400"
                          onClick={() =>
                            setPerfilAbierto(
                              false
                            )
                        ...
                        <button
                          ...
                          className="w-full text-left text-red-400 text-sm font-medium py-2"
                        >
                          Cerrar sesión
                        </button>
                      </>
