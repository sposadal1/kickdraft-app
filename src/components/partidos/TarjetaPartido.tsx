...
const ESTADO_BADGE = {
  ...
  finalizado: {
    label: 'Finalizado',
    className: 'bg-verde-800 text-verde-200',
  },
};
...
export default function TarjetaPartido({
  ...
  return (
    <Link href={`/partidos/${partido.id}`} className="block">
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 hover:border-verde-600 transition-colors cursor-pointer">
        <div className="flex items-center justify-between mb-3">
          <span className="text-xs font-semibold text-verde-400 uppercase tracking-wider">
            {partido.grupoId
              ? `Grupo ${partido.grupoId}`
              : obtenerNombreFase(partido.fase)}
          </span>

          <span
            className={`text-xs px-2 py-0.5 rounded-full font-medium ${estado.className}`}
          >
            {estado.label}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex flex-col items-center gap-1 flex-1">
            ...
            <span className="text-[10px] sm:text-sm font-semibold text-white text-center">
              {equipoLocal.nombreCorto}
            </span>
          </div>
        ...
        <div className="mt-3 pt-3 border-t border-gray-800 flex flex-col sm:flex-row items-start sm:items-center gap-1 sm:gap-4 text-xs text-gray-400">
          <span className="flex items-center gap-1">
            ...
            {formatearFecha(partido.fechaHoraUTC)} ·{' '}
            {formatearHora(partido.fechaHoraUTC)} (COL)
          </span>
